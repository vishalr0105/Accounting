...
export class AccountEndpoint extends EndpointBase {
  ...
  get CompanyRegistration() {
    return this.configurations.baseUrl + '/api/account/CompanyRegistration';
  }
  ...
  getNewUserEndpoint<T>(userObject: any): Observable<T> {
    return this.http
      .post<T>(this.usersUrl, JSON.stringify(userObject), this.requestHeaders)
      .pipe<T>(
        catchError((error) => {
          return this.handleError(error, () =>
  ...
  getPatchUpdateUserEndpoint<T>(
    ...
    if (path) {
      ...
      endpointUrl = opOrUserId
        ? `${this.usersUrl}/${opOrUserId}`
        : this.currentUserUrl;
      patchDocument = valueOrPatch;
    }

    return this.http
      .patch<T>(endpointUrl, JSON.stringify(patchDocument), this.requestHeaders)
      .pipe<T>(
        catchError((error) => {
          return this.handleError(error, () =>
            this.getPatchUpdateUserEndpoint(
              valueOrPatch,
              opOrUserId,
              path,
              from,
